...
- [x] `DifyAgentWorkflow/ai4sec_opencti_webhook_workflow.yaml` 改为直连 OpenCTI GraphQL，不再依赖 `OPENCTI_MCP_URL` `/query`

***人工验收测试步骤 (Generated by Copilot):***
1. 打开 `DifyAgentWorkflow/ai4sec_opencti_webhook_workflow.yaml`，确认环境变量只保留 `OPENCTI_GRAPHQL_URL` 与 `OPENCTI_ADMIN_TOKEN`，且 `query_opencti` 节点的 `url` 指向 `{{#env.OPENCTI_GRAPHQL_URL#}}`。
2. 在同一文件中确认 `query_opencti` 节点标题为 `Query OpenCTI GraphQL`，不再出现 `OPENCTI_MCP_URL` 或 `/query`。
3. 运行 `pytest tests/e2e/test_delivery_assets.py -q`，确认工作流资产测试通过。
- [x] 环境变量命名与 1436 基线一致，优先使用 `OPENCTI_GRAPHQL_URL` 与 `OPENCTI_ADMIN_TOKEN`

***人工验收测试步骤 (Generated by Copilot):***
1. 对照 `DifyAgentWorkflow/ai4sec_threat_modeling_workflow.yaml` 中的环境变量命名，确认 webhook 工作流使用完全一致的 `OPENCTI_GRAPHQL_URL` / `OPENCTI_ADMIN_TOKEN`。
2. 在 Dify 导入工作流后，打开环境变量面板，确认不再需要 `OPENCTI_MCP_URL`。
- [x] Webhook 结果保持现有下游可消费的字段合同，若有差异已显式记录

***人工验收测试步骤 (Generated by Copilot):***
1. 检查 `Build Webhook Response` 节点输出，确认保留了 `scene_type`、`severity`、`task_status`、`name`、`lookup_id`、`bundle_id` 等既有摘要字段。
2. 确认新增字段仅以兼容追加方式出现，例如 `risk_score`、`business_impact_summary`、`priority_recommendation`。
3. 触发一次示例 webhook 后，核对回答文本是否仍包含原有基本摘要，并额外展示新增分析字段。
- [x] 查询字段与关系深度先保持最小闭环，未无控制地扩大全图拉取

***人工验收测试步骤 (Generated by Copilot):***
1. 在 `Normalize Webhook Bundle` 节点中查看 GraphQL 查询，只确认使用 `stixCoreObject(id)` 与一跳 `stixCoreRelationships(fromId/toId)`。
2. 确认查询对象类型仅覆盖漏洞及一跳相关的 `Infrastructure`、`Software`、`Identity` 基础字段，没有引入全图遍历。
- [x] 1215 的其他兼容路径未被删除、未被顺带改写

***人工验收测试步骤 (Generated by Copilot):***
1. 检查 git diff，确认本次没有修改 `mcp/opencti_mcp/app/main.py`、`mcp/opencti_mcp/app/service.py` 等 1215 兼容服务代码。
2. 复查 `DifyAgentWorkflow/ai4sec_unified_workflow.yaml`，确认统一工作流仍保持原来的 MCP 访问方式。
- [x] 超时、重试、错误码实现不超出当前工作流配置能力

***人工验收测试步骤 (Generated by Copilot):***
1. 在 `query_opencti` 节点中确认仍使用原有 `retry_config` 与 `timeout` 字段，没有新增仓外补偿逻辑。
2. 用错误的 `OPENCTI_ADMIN_TOKEN` 触发一次 webhook，确认失败来自 OpenCTI GraphQL 请求本身，而不是新增外部服务。
