...
- [x] 在主分析节点之后新增图表摘要 LLM 节点

***人工验收测试步骤 (Generated by Copilot):***
1. 打开 `DifyAgentWorkflow/ai4sec_threat_modeling_workflow.yaml`，确认 `graph.edges` 中存在 `edge-llm-visual_summary`，并且 `edge-llm-format_threat_model` 的 `source` 为 `llm_visual_summary`。
2. 确认 `graph.nodes` 中存在独立节点 `llm_visual_summary`，位置在 `llm_threat_model` 与 `format_threat_model` 之间。
- [x] 会话结果包含 Mermaid 最小关系图

***人工验收测试步骤 (Generated by Copilot):***
1. 在 Dify 中运行一次 VS1 威胁建模，确认回答中 `Visual Summary` 区块以 ````mermaid` 开头。
2. 核对图中关系仅使用对象名称和 `relationship_type`，没有引入额外图语法噪声。
- [x] 会话结果包含按实体类型分组的 Markdown 表格

***人工验收测试步骤 (Generated by Copilot):***
1. 在同一次 VS1 会话结果中确认 Mermaid 图之后出现多个 Markdown 表格。
2. 核对表格按对象类型分组，且列优先包含 `name`、`id`、`type`、`description`、`relationship_count` 或其降级列。
- [x] `indicator`、`attack path` 缺失时不显示，不补造数据

***人工验收测试步骤 (Generated by Copilot):***
1. 选择一个不含 `indicator` 或 `attack path` 的报告样本执行威胁建模。
2. 确认 `Visual Summary` 中不会出现对应空表或虚构章节。
- [x] 图表结果不写入当前 JSON 下载文件

***人工验收测试步骤 (Generated by Copilot):***
1. 点击回答中的 JSON 下载链接，打开下载内容。
2. 确认下载的 JSON 仍只包含结构化威胁建模结果，不包含 Mermaid 或 Markdown 表格文本。
- [x] 最终展示顺序为图表在前、当前 JSON 在后

***人工验收测试步骤 (Generated by Copilot):***
1. 查看 `Threat Model Output` 节点回答模板，确认 `Visual Summary` 出现在 `analysis_json` 代码块之前。
2. 运行 `pytest tests/e2e/test_delivery_assets.py -q`，确认顺序校验测试通过。
