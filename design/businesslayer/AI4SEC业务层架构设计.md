...
| 平台边界 | OPENCTI 是全局情报底座（1228）；DIFY 仍是唯一保留的智能编排平台，但当前运行态已从统一入口演化为多工作流入口；通知预警标准化通道仍是业务目标，但仓库内未保留独立 Notification MCP 运行时 |
...
| DIFY 平台 | 定义其为业务交互与编排平台，当前可承载多入口工作流 | 不把具体 workflow、Agent 名称或技术节点直接命名成业务能力 |
| 通知协同能力 | 定义其为通知与升级动作的业务能力目标 | 不要求当前仓库必须保留特定 Notification MCP 实现形态 |
