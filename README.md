...
- `/health`: 服务健康检查。
- `/preview`: 生成通知预览。
- `/dispatch`: 生成实际派发响应，默认 `preview_mode=true`。
...
- OpenCTI MCP 默认运行在 mock 模式，未强绑定真实 GraphQL 凭据。
- Notification MCP 默认运行在 preview 模式，不直接投递外部邮件服务。
...
- `OPENCTI_MCP_MOCK_MODE=true`
- `NOTIFICATION_MCP_PREVIEW_MODE=true`
...
并补充真实 OpenCTI GraphQL 地址、访问令牌和通知通道实现。
