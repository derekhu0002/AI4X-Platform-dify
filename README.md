...
- 通过 HTTP 请求节点调用 OpenCTI MCP 的 `/query/threat-model-report` 接口，获取过滤后的 STIX 2.1 bundle 与分析输入。
...
- `/query/threat-model-report`: 面向 VS1 报告驱动威胁建模，支持报告名称/ID 精确匹配优先、双模糊回退、本地别名兜底（用于导入测试数据 ID 映射），并返回过滤后的 STIX 2.1 bundle、analysis_input 与下载文件名。
...
`/query/threat-model-report` 能力摘要：

- 输入：`report_ref`（支持报告名称或报告 ID）。
- 匹配策略：`exact-id` / `exact-name` 优先，无法命中时回退到 `fuzzy-id` / `fuzzy-name`。
- 输出：`matched_report`、过滤后的 `bundle`、`analysis_input`、`download_filename`、`source`。
- 设计目的：为 VS1 workflow 提供“可直接用于 LLM 威胁建模”的最小必要上下文，减少无关字段噪声。
